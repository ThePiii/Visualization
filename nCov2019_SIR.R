...
ggplot(new_cases_df,aes(x=t, y=I)) + geom_point(aes(color=I), size=2) + geom_line(aes(color=I),size=1.5) +
  labs(title = "湖北省百日疫情情况",
       x = "天数",
       y = "受感染人数")
...
ggplot(SIR_reshape, aes(x=time, y=value, group=variable), group_by(variable)) + geom_line(aes(color=variable), size=1.5) +
  labs(title='SIR模型拟合',
      x = '天数',
      y = '比率')

ggplot(fit, aes(x=S, y=I)) + geom_line(color='grey', size=1.2)+
  labs(title='I-S图（相轨线）') + 
